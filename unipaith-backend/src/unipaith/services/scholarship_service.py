...
class ScholarshipService:
    ...
    async def search(
        ...
        *,
        ...
        ``award_type`` is an exact match. Ordered by name. Returns
        ``{"items": [...ORM rows...], "total": int}``."""
        conditions = []
        if q:
            term = f"%{q.strip()}%"
            conditions.append(
    ...
    async def matches_for_student(self, student_id: UUID, *, limit: int = 20) -> list:
        """A "for your level" list. Derives the student's coarse study level
        from real profile signals; if none is known, returns a general first
        page (not a fake match). ``student_id`` is the ``user_id`` of the
        ...
        if level:
            ...
            if result["items"]:
                return result["items"]
        # No derivable level (or no rows at that level) → general first page.
