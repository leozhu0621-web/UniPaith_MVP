"""Specs 48/49/50/51/52 — public build-transparency endpoints.
...
feature coverage, the live route map, the live table map, the acceptance gates),
never any user data. The api-contract and data-model maps are derived from the
running route table and the running SQLAlchemy metadata, so the surface can never
claim something the deployed app doesn't have.
"""
...
@router.get("/overview", summary="Headline build-transparency stats (specs 48–52)")
async def get_overview(request: Request) -> dict:
    """The hub summary: roadmap, feature-coverage, route / table / agent counts and
    the acceptance readiness — everything the ``/goal`` landing needs in one cheap,
    public call."""
