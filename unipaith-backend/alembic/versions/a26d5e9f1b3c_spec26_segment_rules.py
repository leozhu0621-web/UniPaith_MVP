...
1. **Merges the two Spec-24 heads** (``d4e5f6a7b8c0`` dataset versions/templates
   and ``f24da7a0c1b3`` data upload) back into a single linear head so
   ``test_alembic_has_single_head`` passes.
2. **Extends ``target_segments``** with the Spec 26 §7 ``Segment`` shape — a
   nested include/exclude ``rules`` tree (superseding the legacy flat
   ``criteria``), uploaded prospect-list references, a per-segment weekly send
   cap, the author, and a cached preview count.
3. **Widens ``ck_ai_turns_agent``** to admit the new ``segment_builder_nl`` agent
   (Spec 26 §6 / 45 §17).
...
Revises: d4e5f6a7b8c0, f24da7a0c1b3
...
down_revision = ("d4e5f6a7b8c0", "f24da7a0c1b3")  # pragma: allowlist secret
branch_labels = None
depends_on = None


# Current vocabulary (post Spec 24) + the new segment_builder_nl agent.
_AGENT_CHECK_NEW = (
    ...
    "'document_parse_triage','segment_builder_nl')"
)
_AGENT_CHECK_OLD = (
    "agent IN ('orchestrator','extractor','validator','feature_emitter',"
    "'rationale','workshop_coach','workshop_judge','embedding',"
