...
# Rebased onto the current single head after Spec 12's School-Detail backfill
# (b12c0de5f7a9, #236) merged ahead of this branch — it chains off the Spec 40
# head (r40a1b2c3d4e) and adds no AI agents, so the agent CHECK constants below
# still extend the post-Spec-40 vocabulary. Chaining off b12 keeps the graph
...
_AGENT_CHECK_NEW = (
    ...
    "'prospect_prioritizer','territory_optimizer','prompt_coach')"
)
# Prior state (the down_revision's vocabulary — Spec 40 set).
_AGENT_CHECK_OLD = (
    "agent IN ('orchestrator','extractor','validator','feature_emitter',"
    "'rationale','workshop_coach','workshop_judge','embedding',"
    "'review_summarizer','authenticity_risk','matcher','query_interpreter',"
