...
Chains off ``a32revwork1b2c`` (the Spec 32 head) so the graph stays a single
...
down_revision = "a32revwork1b2c"  # pragma: allowlist secret
branch_labels = None
depends_on = None


# Full post-Spec-33 vocabulary (adds the two interview agents on top of the
# Spec 32 review-assist agents).
_AGENT_CHECK_NEW = (
...
# Prior state (post Spec 32 — includes the review-assist agents, not yet the
# interview agents).
_AGENT_CHECK_OLD = (
    "agent IN ('orchestrator','extractor','validator','feature_emitter',"
    "'rationale','workshop_coach','workshop_judge','embedding',"
