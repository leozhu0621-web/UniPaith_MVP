...
async def test_create_institution(institution_client: AsyncClient):
    resp = await institution_client.post("/api/v1/institutions/me", json={
        "name": "New University",
        "type": "university",
        "country": "United States",
    })
...
async def test_update_institution(
    ...
    await _ensure_institution(db_session, mock_institution_user)
    resp = await institution_client.put("/api/v1/institutions/me", json={
        "city": "Boston",
    })
    assert resp.status_code == 200
...
async def test_create_program_draft(
    ...
    await _ensure_institution(db_session, mock_institution_user)
    resp = await institution_client.post("/api/v1/institutions/me/programs", json={
...
async def test_publish_program_missing_fields(
    ...
    await _ensure_institution(db_session, mock_institution_user)
    create_resp = await institution_client.post("/api/v1/institutions/me/programs", json={
        "program_name": "MS in CS",
        "degree_type": "masters",
    })
...
async def test_public_search_only_published(
    institution_client: AsyncClient, client: AsyncClient,
    db_session: AsyncSession, mock_institution_user: User,
):
    await _ensure_institution(db_session, mock_institution_user)
    await institution_client.post("/api/v1/institutions/me/programs", json={
...
async def test_public_search_filter_by_degree(
    ...
    for name, deg in [("MS CS", "masters"), ("PhD CS", "phd")]:
        r = await institution_client.post("/api/v1/institutions/me/programs", json={
            "program_name": name, "degree_type": deg,
            "description_text": "test", "tuition": 10000,
        })
        await institution_client.post(
            f"/api/v1/institutions/me/programs/{r.json()['id']}/publish"
        )
...
async def test_segment_crud(
    ...
    resp = await institution_client.post("/api/v1/institutions/me/segments", json={
    ...
    resp = await institution_client.put(f"/api/v1/institutions/me/segments/{sid}", json={
