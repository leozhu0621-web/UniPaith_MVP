"""Berkeley profile — professional-tier tuition coverage (REPAIR_BACKLOG #4)."""

from unipaith.data import berkeley_profile as p
...
def test_professional_tier_carries_published_tuition():
    ...
    for spec in professional:
        ...
        assert cost["tuition_usd"] != p._TUITION_IN_STATE, spec["slug"]
...
def test_professional_tuition_rates_are_distinct():
    ...
    assert len(prof_rates) >= 7
    assert p._TUITION_IN_STATE not in prof_rates
