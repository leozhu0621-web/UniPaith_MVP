# 30 · Institution Setup (First-Run Wizard)

> The institution onboarding flow at `/i/setup` — the guided multi-step wizard a new institution completes before reaching the dashboard. Expands the first-run sketch in `05` §11 into a build contract. Orchestrates the already-specced editors (`22` profile, `23` programs, `24` data) into a sequenced first-run experience.
>
...
A reduced form over the profile editor (`22`): legal/display name, type (community college / regional public / …), location, primary domain (for SES sender + trackable links), short description, logo (text wordmark per brand — no decorative imagery).
→ Produces a minimally publishable institution profile.

### 3.2 Step 2 — Your first program
A reduced slice of the program editor (`23`): name, degree type, modality, one intake/deadline, requirements summary, cost basics. "Add more details later."
→ Produces one publishable `ProgramCard` so the institution is matchable.

### 3.3 Step 3 — Upload data (optional, skippable)
A pointer into Data Upload (`24`): "Upload admissions history or a prospect list to power analytics + matching." Clearly skippable ("I'll do this later") — never blocks completion.
→ Seeds the value-for-data loop (`07` §4.3) when provided.

### 3.4 Step 4 — Invite your team (optional)
Invite colleagues with roles (admissions / recruiter / marketing / IT) — a slice of `21` §3.1. Skippable.
→ Sets up collaboration; each invite audit-logged (`36`).
...
- The dashboard (`31`) shows a setup-progress card until profile + program are published; skipped optional steps surface as dashboard nudges, not blockers.
...
```
Endpoints: `GET /i/setup` · `PATCH /i/setup/step` · `POST /i/setup/complete`. Each step delegates to the underlying editor's endpoints (`22`/`23`/`24`/`21`), so there is no duplicate persistence.
...
- Optional: `CampaignAudienceCopySuggester` (`45` §16) can draft the institution description in Step 1; institution edits before saving. No AI is required to complete setup.
...
- Mobile: single-column steps, sticky Continue bar (`03` §7).

---

## 9. Gaps (relative to current code)

- A wizard exists in spirit (`05` §11) but is not built as a resumable, step-persisted flow.
- `setup_complete` gating flag is NEW.
- Reuses `22`/`23`/`24`/`21` editors — those must expose "reduced/first-run" variants of their forms.
...
- **Student onboarding parity.** The student first-run counterpart is specced in `19` (Stage-1 discovery chat) + `05` §4.5/§11 — see those, not this doc.
