# 17 · Applications — Per-Application Workspace
...
The single most important spec element. The institution publishes a `requirement_checklist` per program (per `21-program-detail-page-institution.md`). UniPaith renders it consistently:
...
**"Ready to submit" gate enables only when all required items pass.** Per the Adaptive Intake Engine apply-ready check (`41-adaptive-intake-engine.md` §4.2).
...
### 6.2 Essays
Per-application essay list. CRUD. Each essay → "Get feedback" routes to Workshops (`16`) program-specific mode.
...
### 6.5 Guardrails (CURRENTLY UNWIRED — `90` G-S4)
...
Accept / decline. Deadlines visible. Offer terms summary via `OutcomeBriefForOfferLetter` agent (`42` §15).
...
- Priority must be set (Reach / Target / Safer) per saved list (`15`).
...
| (New) `OutcomeBriefForOfferLetter` (`42` §15) | On offer received | Plain-language brief |
