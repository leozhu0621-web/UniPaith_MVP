...
Agent: `WorkshopCoach (Essay variant)` (`42-ai-agents-claude.md` §7).
...
Agent: `WorkshopCoach (Interview variant)` (`42` §8).
...
Agent: `WorkshopCoach (Test variant)` (`42` §9).
...
All gated on `ai_workshops_v2_enabled` feature flag. Per `03-llm-claude-migration.md` migration plan.
...
All three coaches per `42-ai-agents-claude.md`. Sonnet tier (workhorse). Cache: system block at 1h; persona at 5min; tail = the artifact uncached.
