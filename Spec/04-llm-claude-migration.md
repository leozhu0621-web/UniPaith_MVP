# 04 · LLM Migration — OpenAI → Claude (Anthropic)
...
New agents to add (covered in `45-ai-agents-claude.md`):

| # | Future agent | Tier | Purpose |
|---|---|---|---|
| 11 | DiscoveryQueryInterpreter | Sonnet | Convert student-typed NL query → constraint chips for Discovery (spec `10`). |
| 12 | InboxReplyDrafter | Sonnet | Suggest a draft reply to admissions-officer threads. Human accepts/edits before send. |
| 13 | DraftSummarizerForReview | Opus | Per-application packet summary for institution reviewers (`32-review-workspace.md`). One Opus call per application; cached per `(profile_v, program_v, intake_id)`. |
| 14 | OutcomeBriefForOfferLetter | Sonnet | Plain-language brief on a decision/offer for students. |
| 15 | CampaignAudienceCopySuggester | Sonnet | Draft external email body/subject for institution Campaigns (`25`). |
...
This ledger is the source for: cost dashboards, compliance audits, fairness/bias reporting (per `46-data-rights-privacy.md`), and per-cohort halt logic (per `46` §6).
...
- The active prompt version changes (prompt iterations stored in `45-ai-agents-claude.md` and version-tagged in code).
...
- **Per-institution model isolation.** If a partner institution demands that THEIR data never touches the multi-tenant cache, we'd need per-tenant cache keys and possibly a per-tenant Claude-on-Bedrock account. Spec'd in `46-data-rights-privacy.md`.
