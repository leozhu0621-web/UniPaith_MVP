...
> The money layer: application-fee collection, fee-waiver workflow, enrollment-deposit collection, and refunds. The real payment gateway behind the status-only stubs in `15` §2A (application cost tracker) and `35` (deposit status). Until this ships, those surfaces record status without moving money.
>
> Status: **draft v1.0 · Phase-2** · 2026-05-29 · Routes: student inline at `/s/applications/:appId` (pay fee) + `/s/applications/:appId?tab=enrollment` (pay deposit); institution `/i/settings?tab=billing` (fee config) + `/i/admissions` (waiver queue). Scope: ships when the first institution requires real collection (`35` §12).
...
2. **Enrollment deposits** — an admitted student pays a deposit to confirm enrollment (`35`).

Plus the platform's own student subscription billing (`07` §4.1) and institution usage billing (`07` §4.2) — those are covered in `21` §2.7 / §3.6; this doc is the **applicant-facing transactional** layer.
...
Per program/intake (`23`): fee amount + currency, or "no fee." Fee-waiver policy: auto-eligible rules (income band, fee-waiver code, first-gen, SRAR/NACAC waiver) + manual-request path.

### 2.2 Student flow
- At submit (`15`), if the program has a fee: a pay step (or "request waiver").
...
- Receipt + history in the application cost tracker (`15` §2A).

### 2.3 Waiver queue (institution)
- Pending waiver requests with basis + supporting evidence.
- Approve / deny / request-more-info. Audit-logged (`36`).
...
- Configured per program/intake (`23`): deposit amount, deadline, refundability rules.
- Student pays from the enrollment window (`35` §2.1) → `deposit_status=paid`, enrollment state advances.
- Waiver/deferral of deposit per institution policy.
- Deposit feeds yield (`35` §4).

---

## 4. Payments infrastructure

- **Gateway:** Stripe (cards) + ACH for larger deposits; provider abstracted behind a `PaymentProvider` interface (mirrors the model-portability pattern in `04` — swap providers, not call sites).
- **PCI:** never store raw card data; use the gateway's tokenized elements. Only store provider charge IDs + status.
- **Money movement:** platform processes on behalf of the institution (Stripe Connect — funds route to the institution's connected account, platform takes its fee per `07` §4.2).
- **Idempotency:** every charge keyed by `(application_id, fee_type)` to prevent double-charge on retry.
- **Webhooks:** gateway webhook updates `fee_paid` / `deposit_status` (the status fields `15`/`35` record manually today).
...
- Never dark-pattern a fee over a waiver — the waiver path is equally prominent (equity value, `07` §2).

---

## 9. Gaps / dependencies

- Replaces the status-only stubs in `15` §2A + `35` (deposit). Those docs explicitly defer real money here.
...
- Deposit payment advances enrollment state (`35`) + increments yield.
...
- **Subscription billing reuse** — does student `$15/mo` (`21` §2.7) share this provider layer? Yes — same `PaymentProvider`.
