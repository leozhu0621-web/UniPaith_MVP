# 06 · Product Context — Positioning, GTM, Pricing, Competitive Moat
...
1. **Fit, not fame** → matching optimizes thrive-probability, not brand rank (`11` band logic).
2. **Explain everything** → every score/rank/recommendation ships with reasoning (`42` rationale agents, `02` §15).
3. **Partnership, not extraction** → value-for-data; raw student data never sold (`43` §3).
4. **Bias-avoidance is a practice** → cohorts audited, auto-halt at Δ>0.20/2wk, decisions never fully automated (`43` §6).
...
**Product implication:** the MVP-core (specs `10`–`35`) serves the beachhead. The deferred enterprise depth in `92-feature-backlog.md` (I-20 generation, IPEDS, faculty-advisor matching, territory CRM) is up-market scope — build it when up-market institutions demand it, not before.
...
**Spec decision:** the Master Paper's $15/mo + 7-day-trial + card-on-file is the **current** model (most recent doc). The freemium feature-gating from the Business Plan informs WHAT's free vs paid within that model. Affects: auth/paywall flow (`04` §9 — trial→paywall), feature gating across `10`–`1B`.
...
**Spec decision:** $15/applicant is the **headline** model. The modular add-ons map to the deferred `92` features (reviewer seats → blind review/calibration; analytics → `26`; verification → integrity; integrations → SIS). Build the core; the add-ons become upsell SKUs.
...
Participation tiers: **"no-training"** ←→ **"model partner"** (`43` §9). Governance: written DPAs, institutional control, student consent, privacy-by-design. This is the moat (§6) and the basis for the `consent.training` lever (`43` §2).
...
- Figuring out **which schools** to consider → Match/Discovery (`11`, `12`).
- Understanding **requirements** → Detail Pages + adaptive checklist (`13`, `17`).
- **Essays** → Workshops (`16`).
- **Deadline/paperwork management across schools** → Calendar + Applications (`17`, `18`).
- Evaluating **aid/cost** → Net Price Estimator (`92` extend), Costs tab (`13`).
- Getting **relevant advice** → Discovery chat (`1B`).
- Tracking **status** → Applications + Inbox (`17`, `19`).
- **One place to manage the whole process** (Q16) → the platform thesis itself.

AI-trust factors the market cares about (Q14): **transparency, human-final-decision, privacy, workload reduction, accuracy, equal access**. Top AI concerns (Q15): won't understand individual circumstances, data privacy, bias, over-reliance, impersonal. → Directly validates the explainable-rationale + human-in-the-loop + fairness-auto-halt design (`35`, `42`, `43`).
...
**Must-do integrations** (from the verdicts): integrate WITH Slate (don't replace the system-of-record); wrap Common App (don't compete on the submission rail); integrate peer-engagement (don't rebuild Unibuddy). See `90` G-A / `05` §4.
...
Competitor outcome benchmarks stakeholders will ask about (from `Competition Analysis`): Mainstay/Pounce −21.4% summer melt, +3.3% enrollment; Unibuddy 51% chat-prospects-apply; Niche Premium 2–4× CTR; Element451 −24% call volume / +10% enrollment. UniPaith's analytics (`26`) should be ready to report comparable numbers.
...
**Product implication:** the Universal Profile (`10`) + Prompt Library (`40`) + verification/integrity signals (`40` §3.17, §4.16) are the **defensible core** that survives even a pivot. Prioritize their quality. Everything else is the full-platform bet on top.
...
- **Analytics targets:** §7 → what `26` must be able to report.
...
- **Institution add-on SKUs** (§4.2): which deferred `92` features become paid add-ons vs included.
- **Funding figure** (§8): confirm $4.0M is current.
- **US-first vs global** (§10): confirm execution scope.
- **Roadmap ordering** (`92` §6): chat-first (shipped) vs structured-Match-first (founder roadmap).
