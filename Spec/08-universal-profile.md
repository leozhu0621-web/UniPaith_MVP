# 08 · Universal Profile — Student Workspace
...
>
> Status: **draft v1.0** · 2026-05-29 · Routes: `/s/profile` + `?tab=*` deep links · Depends on `01-brand-tokens.md`, `02-design-system.md`, `42-prompt-library-schema.md`, `44-adaptive-intake-engine.md`.
...
Edit triggers a sheet (per `02` §6) with all `Identity, contact, account` fields per `42-prompt-library-schema.md` §3.1.
...
Sourced from `next_questions_to_ask_user` + derived `recommended_next_actions` (per `42` §4.14). Tappable; each opens the relevant tab or modal.
...
Generated by `IdentitySummaryAgent` (`45` §11). The Regenerate button calls `/me/identity/regenerate-summary` (existing endpoint). On failure, preserves the previous real summary; never overwrites with stub.
...
### 6.2 Test Scores
Per `42` §3.6. Each test = one card. Sections + subscores listed under each. Superscore computed automatically (`superscore_computed_values`). Verified status badge.

### 6.3 Languages
Per `42` §3.11. CEFR or ACTFL proficiency picker. Proof type. Can-demonstrate toggle. List view; one row per language.

### 6.4 Research
Per `42` §3.9 (research sub-list). One card per project. Outputs (paper / poster / code / dataset / presentation) as chips. Publication links validated.
...
### 7.1 Activities
Per `42` §3.9 (activity-shaped entries). One card per activity. Category, role, leadership level, scope, dates, hours.

### 7.2 Work & Service
Per `42` §3.8. One card per role. Internship / part-time / full-time / volunteer / fellowship.

### 7.3 Competitions
Per `42` §3.9 sub. One card per competition.

### 7.4 Portfolio
Per `42` §3.10. Top-level fields + grid of pieces. Each piece: thumbnail (where possible), title, type, tags. Add piece via drag-drop or link.

### 7.5 Online Presence
Per `42` §3.1 online links subset. LinkedIn, GitHub, personal site, others. URL validation status badges.

---

## 8. Goals tab

Per `42` §3.12 + the existing `student_goals` model.
...
Per `42` §3.13 + `student_needs` model.
...
Generated by `StrategyAgent` (`45` §10).
...
### 11.2 Accommodations
Per `42` §3.2. Free-form details + documentation status. Privacy-gated.

### 11.3 Scheduling
Available times for interviews / advising / visits. Weekly recurrence grid + per-date overrides.

### 11.4 Recommenders
Per `42` §3.7. One card per recommender. Status pipeline (not_requested → requested → in_progress → submitted). Request action sends an email; tracks via `recommendation_platform`.

---

## 12. Preferences tab

Per `42` §3.14. Structured pickers:
...
Per `46-data-rights-privacy.md` §8 student rights surface.

- **Consent toggles** (4 levers per `46` §2). Each toggle: 1-line explanation + "Last changed" + change history link.
- **Portable export** — JSON (full structured) + PDF (human-readable) of every signal.
- **Common App / Coalition format export** (MVP-extend, `49` §2) — maps the Universal Profile onto the Common App / Coalition field schema so the student can carry their work into those platforms (the "apply once, go anywhere" payoff). A field-mapping layer translates `profile field → Common App field`; unmapped fields are listed so nothing is silently dropped. Read-only in MVP (no write-back API to Common App).
- **LinkedIn import** (MVP-extend, `49` §2) — pre-fill Work / Education / Skills via the external-link channel (`44` §5.3); imported fields land `source: student-link`, confidence 75, student-confirmable. One-way (LinkedIn → UniPaith) in MVP.
...
Backend reads/writes via the Prompt Library schema (see `42-prompt-library-schema.md` §8). Frontend API client per existing patterns in `frontend/src/api/`.
...
The full list of types maps 1:1 to `42` field tables.
...
|---|---|---|---|
| Identity tab → AI summary | `IdentitySummaryAgent` (`45` §11) | Initial + Regenerate button | Inline card; AI badge per `02` §15 |
| Strategy tab → Generated strategy | `StrategyAgent` (`45` §10) | First-time generate + Regenerate button | Inline card; AI badge + Why popover |
...
Per `47-current-vs-spec-gap-audit.md` G-S1:
...
4. Build Data Rights tab per `46` §8.
5. Build Timeline tab.
6. Migrate Essays & Resume legacy out (per `14-workshops.md`).
7. Wire `consent.training` lever (per `46` §2).
