# 91 · Build Sequencing — Phased Roadmap from Current MVP → Master Paper Spec
...
>
> Status: **draft v1.0** · 2026-05-29 · Depends on `90-current-vs-spec-gap-audit.md` for gap detail.
...
**Specs:** `03-llm-claude-migration.md`, `42-ai-agents-claude.md`.
**Gap items:** G-AI1, G-AI2.
**Effort:** 8 days.

**Steps:**
1. Add `anthropic` Python SDK to `unipaith-backend/pyproject.toml`.
2. Build `services/ai/providers/{base.py, anthropic.py, openai.py, registry.py}` per `03` §5.
...
**Done when:** route map matches `04-information-architecture.md` exactly; codebase has ~3,000 fewer lines; lint clean.

---

## 7. Phase 4 — Data spine (Prompt Library)

**Goal:** all input categories from `40-prompt-library-schema.md` have models, migrations, and CRUD endpoints. Major-specific tracks are JSONB-stored. Output schema columns on `ai_artifacts`.

**Specs:** `40-prompt-library-schema.md`, `41-adaptive-intake-engine.md`.
**Gap items:** G-D1, G-AI3.
**Effort:** 8–10 days.

**Steps:**
1. Per `40` §8, add the NEW tables and extend the existing ones.
...
**Specs:** `12-discovery.md`.
**Gap items:** G-S3, G-AI6.
**Effort:** 4 days.

**Steps:**
1. New agent `DiscoveryQueryInterpreter` per `42` §11 + spec.
...
**Specs:** `11-program-match.md`, `13-detail-pages-program.md`, `15-saved-list.md`.
**Gap items:** G-S2.
**Effort:** 2 days.
...
**Specs:** `17-applications.md`, `16-workshops.md`.
**Gap items:** G-S4, G-A5.
**Effort:** 3 days.
...
**Specs:** `21-program-detail-page-institution.md`.
**Gap items:** G-I1.
**Effort:** 4 days.
...
**Specs:** `43-data-rights-privacy.md`.
**Gap items:** G-I5, G-D4.
**Effort:** 5 days.
...
**Specs:** `35-ai-extensibility.md`, `42-ai-agents-claude.md`.
...
Per /Users/leozhu/Desktop/工作/UniPAith/Spec/91-build-sequencing.md §<N>,
...
- 01-brand-tokens.md
- 02-design-system.md
- 03-llm-claude-migration.md
- 04-information-architecture.md
- <phase-specific spec docs>
- 90-current-vs-spec-gap-audit.md
- 91-build-sequencing.md
```
...
| Claude API quota/limit | `/health/ai` probe + provider failover to OpenAI per `03` §9. |
