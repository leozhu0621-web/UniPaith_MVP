# 21 · Program Detail Page Editor (Institution-Facing)
...
>
> Status: **draft v1.0** · 2026-05-29 · Routes: `/i/programs/:id/edit`, `/i/programs/new`. Public output renders at `/s/programs/:programId` (`13`) and `/program/:programId`.
...
type ProgramApplicationRequirements = {
  ...
  test_policy: {
    ...
    stance: 'required' | 'recommended' | 'test_optional' | 'test_blind';   // MVP-extend, 92 §3
...
// Test-score management (MVP-extend, `92` §3): `test_policy.stance` drives the student's
// `test_policy_compatibility` (`40` §4.6) + apply-ready gate (`40` §6.2). `superscore_enabled`
// → platform computes the superscore across attempts (`40` §4.6). Test-optional cohort
// outcome-analysis for the committee lives in `31` §7A.3. Direct score import + verification
// record status in MVP; provider integration is Phase-2 (`92`).
