...
### 2.5 Uploaded lists
- CSV imports from `24-data-upload.md`.
...
`SegmentBuilderNLBridge` agent (`45` §17) — convert natural language ("students who saved Engineering programs in California with budget ≤ $40k") to structured rules. Confidence + ambiguity notes shown; user edits before saving.
...
- `SegmentBuilderNLBridge` agent doesn't exist; build per `45` §17.
...
- **Per-segment fairness check.** Per `46` §6 — if a segment heavily skews on a protected attribute, surface a warning before send.
