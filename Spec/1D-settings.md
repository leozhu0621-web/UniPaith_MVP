...
> Account, security, locale, notification preferences, data-rights entry, and account deletion — for both roles. The dedicated home for the surfaces previously sketched in `04` §10. Routes: `/s/settings` (student, under the avatar dropdown) and `/i/settings` (institution).
>
> Status: **draft v1.0** · 2026-05-29 · Supersedes the `04` §10 table (which now points here). Gives `/s/settings` + `/i/settings` a dedicated build contract (previously a section in `04` §10).

---

## 1. Purpose

One predictable place for everything that isn't the product surface itself: who you are, how you sign in, how you're notified, what language/timezone you see, and how you leave. Notification preferences live **here**, not on Profile (one-place-for-notifications principle, `04` §6.6).
...
- **Login activity** — recent logins + `login_risk_events` (`40` §3.17) surfaced read-only.

### 2.3 Preferences
- **Locale** (language) — drives `preferred_platform_writing_language` (`40` §3.1).
- **Timezone** — drives all deadline/calendar normalization (`18`, `40` §4.1 `timezone_normalized_deadline_calendar`).
- **Accessibility** — dyslexia-friendly mode, font size, reduced-motion (`02b` §9, `40` §3.1 accessibility fields).
...
- Types: match updates, application missing-item, interview invites, deadline reminders, decisions, institution posts on saved programs (`19`/`1C`/`18` triggers, mapped in `04` §12).
- Email frequency: all / weekly digest / important only / none (`40` §3.1).
- Each toggle persists to the student's notification-prefs; transactional/active-application messages can be down-ranked but not fully silenced (safety — see `27` §6).

### 2.5 Data & privacy (entry point)
- The full consent panel + portable export + access log live on **Profile → Data tab** (`10` §16, `43` §8). Settings shows a summary + a "Manage data rights →" link there (avoid two sources of truth).
- **Account deletion** — "Danger zone" card: triggers soft-delete + 30-day grace + full purge (`43` retention). Requires re-auth + typed confirmation.

### 2.6 Sign out
- Sign out (this device) + "sign out everywhere."

### 2.7 Billing (student, if subscribed)
- Plan status ($15/mo + trial state, `06` §4.1), card-on-file, invoices, cancel/downgrade, ad-free upgrade toggle. (Payments provider integration — Phase-2 detail; MVP shows plan state + manage link.)
...
- **Team / seats** — list staff, invite, role assignment (admissions / recruiter / marketing / IT), deactivate. Audit-logged (`34`). (Multi-institution switching is post-MVP, `04` §5.)
...
- **Rubric management** — define/edit the scoring rubric used in `31` (criteria, weights, scales).
- **Blind-review default** + **calibration settings** (`31` §7A.1/§7A.2) toggles at the institution level.
- Reviewer-assignment defaults.

### 3.4 Integrations
- **Credentials** for SIS/CRM (Slate, Salesforce, Banner/Workday) + API webhooks (`05` §4; most deferred to Phase-2 per `92`).
- SES sender domain verification status (`23`).
- Data export / dataset access (`22`).

### 3.5 Notifications (institution)
- Per-type institution alerts (new inquiry, new application, reviewer overdue, yield-risk, disparate-impact threshold — `04` §12, `43` §6) × channel.

### 3.6 Billing
- Usage-based billing ($15/applicant, `06` §4.2): current cycle usage, invoices, payment method, plan/add-ons.
...
Most fields map to `40` §3.1/§3.2 (contact, accessibility, consent, communication prefs) — Settings is a focused editor over a subset of the Prompt Library + Cognito-managed auth fields.
...
- Password change / MFA / locale / timezone / deletion are TODO in the current build (`04` §10 marked them TODO).
...
- Notification toggle persists + governs delivery (cross-check `04` §12 triggers).
- Password change + MFA enroll/disable round-trip via Cognito.
- Timezone change re-normalizes calendar deadlines (`18`).
- Account deletion → 30-day grace state + undo; purge after grace (`43`).
