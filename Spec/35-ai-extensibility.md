# 35 · AI Extensibility — Assistive Layer for Admissions

> The contract for how AI plugs into procedural workflows. AI generates drafts and operational suggestions; humans keep final action. The principle that governs every AI-touched surface in `30`, `31`, `32`, `33`, `19`, `23`.
>
...
### 2.1 Drafts
- **Packet summary** (`31` §6) — `DraftSummarizerForReview` (Opus).
- **Rubric pre-fill** (`31` §6) — `AIPrefillForRubric` (Sonnet).
- **Decision notice draft** (`33`) — AI draft message before staff edits + sends.
- **Missing-items message** (`30`) — AI draft before send.
- **Interview-invite message** (`32`) — AI draft.
- **Clarification request** (`30` / `19`) — AI draft.
- **Inbox reply suggestion (student)** (`19`) — `InboxReplyDrafter`.
- **Campaign copy** (`23`) — `CampaignAudienceCopySuggester`.
- **Segment NL → rules** (`24`) — `SegmentBuilderNLBridge`.

### 2.2 Operational suggestions
- **Queue prioritization** (`30`) — AI-ranked priority queue.
- **Yield-risk alerts** (`30`) — AI-driven detection.
- **Intelligence digest** (`30`) — daily AI-narrated summary.
- **Drop-off diagnosis** (`26`) — AI-explained funnel drop.

### 2.3 Integrity / triage
- **Authenticity risk scoring** (`42` §18) on essays.
- **Document parse triage** (`42` §19).
...
Per `31` §6 — Q&A about the applicant. Sonnet. Persona = full applicant packet.
...
- "No-training tier" override (per `43` §9) — block all data from training corpus.
...
- G-AI5 (major): DraftSummarizerForReview needs explicit wiring to Opus per `42` §14.
