# 42 · Prompt Library — Canonical Signal Schema
...
>
> **Used by:** every spec that touches student data (`08`, `09`, `10`, `11`, `13`, `14`, `15`, `18`, `19`, `24`, `31`, `32`, `37`, `44`, `45`, `46`).
...
| `consent_use_for_model_training` | bool | DEFAULT false. **The 4th lever.** Enforced per `04-llm-claude-migration.md` §11. |
...
The standalone Prompt Library defines a structured behavioral-prompt system — distinct from Workshops (`14`). Workshops give feedback on a draft; this layer is the **catalog of prompts** + the student's **responses** as reusable profile signals. Stored in `student_behavioral_prompts` + `student_behavioral_responses`.
...
Inference layer responsibilities. Each output is computed by one or more of the agents in `45-ai-agents-claude.md`. Each is stored with provider+model+timestamp+confidence per `04` §8.
...
| `ai_artifacts` | §4 outputs — JSONB per category | exists; extend per `04` §8 |
...
- **Major-specific catalog full reproduction.** Move §3.18 to its own doc `43-prompt-library-major-specific.md` for editability.
- **Privacy/PII labels.** Each field marked `pii-sensitive` or stricter needs an explicit retention rule. Cross-reference with `46-data-rights-privacy.md`.
...
- **CSV import column maps.** For `24-data-upload.md`, the institution-side bulk import must declare column → Prompt Library field. A reverse lookup table per data source type would help.
