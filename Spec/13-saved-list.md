...
### 4.2 Priority labels
Per saved program: `considering` (default), `planning_to_apply`, `applied`, `dropped`. Persisted server-side (per `47` G-S5 — currently UI-only; needs backend wiring).
...
Same compare tray as `10-discovery.md` §8. Select multiple rows → "Compare selected (N)" CTA → side-by-side comparison.
