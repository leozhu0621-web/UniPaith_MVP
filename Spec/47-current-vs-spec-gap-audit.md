# 47 · Current MVP vs Spec — Gap Audit
...
**Severity:** `major` · **Type:** `migrate` · **Owner:** `11-detail-pages-program.md` · **Effort:** 0.5 day
...
**Severity:** `minor` · **Type:** `remove` · **Owner:** `10-discovery.md` · **Effort:** 0.5 day
...
**Severity:** `minor` · **Type:** `remove` · **Owner:** `09-program-match.md` · **Effort:** 0.5 day

Not in `App.tsx` routes. Replaced by Discover + Explore. Delete.

### G-A4 · Five dead student pages
**Severity:** `minor` · **Type:** `remove` · **Owner:** `05` · **Effort:** 0.5 day
...
**Severity:** `defer` · **Type:** `remove` · **Owner:** `14-workshops.md` · **Effort:** 0.5 day
...
**Severity:** `major` · **Type:** `extend` · **Owner:** `08-universal-profile.md` · **Effort:** 3 days

Current `ProfilePage.tsx` has 7 tabs (Overview, Identity, Goals, Needs, Strategy, Essays & Resume legacy, Recommenders, Financial). The Overview tab itself spans 18 of 19 spec sections. The spec calls for 13 cluster tabs (per `05-information-architecture.md` §4.6) plus Notifications under Settings. The 19th section (Analytics) is missing.

**Action:**
1. Implement Analytics tab content (profile completeness over time, signal-density-by-category chart, peer-comparison view).
2. Reorganize tabs per `05` §4.6 (collapse Overview's 18-section super-scroll into discrete tabs).
3. Migrate Recommenders + Financial into the Preparation tab.
4. Migrate Essays & Resume legacy out — `14-workshops.md` is the destination.

### G-S2 · Dual-score (fitness + confidence) not surfaced where it matters
**Severity:** `major` · **Type:** `wire` · **Owner:** `09-program-match.md`, `11-detail-pages-program.md`, `13-saved-list.md` · **Effort:** 2 days
...
**Severity:** `major` · **Type:** `extend` · **Owner:** `10-discovery.md` · **Effort:** 3 days
...
**Action:**
1. New agent `DiscoveryQueryInterpreter` per `45-ai-agents-claude.md` returns structured constraints, not free-text summary.
...
**Severity:** `major` · **Type:** `wire` · **Owner:** `15-applications.md` · **Effort:** 2 days
...
**Severity:** `major` · **Type:** `wire` · **Owner:** `13-saved-list.md` · **Effort:** 0.5 day
...
**Severity:** `major` · **Type:** `migrate` · **Owner:** `19-discovery-stage-conversation.md`, `05` · **Effort:** 1 day
...
**Severity:** `major` · **Type:** `new` · **Owner:** `05` (Connect surface), pending new spec · **Effort:** 5 days

Spec calls Stage 3a "Connection & Outreach" — Peers is core to it.

**Action:** Define peer model (opt-in profile sharing per program), API, and tab UI in a new spec doc `20-connect.md`. Defer until institution-side relationships are clearer.

### G-S8 · Student SettingsPage missing locale/MFA/password/account-delete
**Severity:** `major` · **Type:** `extend` · **Owner:** `05` §10 · **Effort:** 3 days
...
**Severity:** `minor` · **Type:** `migrate` · **Owner:** `11`, `12` · **Effort:** 1 day
...
**Severity:** `major` · **Type:** `extend` · **Owner:** `23-program-detail-page-institution.md` · **Effort:** 4 days
...
**Severity:** `minor` · **Type:** `migrate` · **Owner:** `28-attribution-funnel-analytics.md` · **Effort:** 2 days
...
**Severity:** `minor` · **Type:** `wire` · **Owner:** `23` · **Effort:** 0.5 day
...
**Severity:** `minor` · **Type:** `extend` · **Owner:** (institution messaging — covered in `17`-analog) · **Effort:** 3 days
...
**Severity:** `block` for `46-data-rights-privacy.md` · **Type:** `new` · **Owner:** `46`, `28` · **Effort:** 5 days
...
**Severity:** `block` for user directive · **Type:** `migrate` · **Owner:** `04-llm-claude-migration.md` · **Effort:** 8 days (full migration), 1 day per agent

10 existing agents — see `04` §4. Plan: add Anthropic provider, then port one agent at a time behind existing flags.

### G-AI2 · AI audit ledger doesn't record provider
**Severity:** `major` · **Type:** `extend` · **Owner:** `04` §8 · **Effort:** 1 day
...
**Action:** Add columns per `04` §8 schema. Existing rows backfill provider=`openai`.

### G-AI3 · Consent mask has no "training" dimension
**Severity:** `major` · **Type:** `extend` · **Owner:** `46-data-rights-privacy.md` · **Effort:** 1 day
...
**Action:** Add `consent.training` boolean to `student_profiles`; expose in Profile Data Rights tab; enforce in every agent call site per `04` §11.

### G-AI4 · Authenticity risk scoring not implemented
**Severity:** `major` · **Type:** `new` · **Owner:** `37-ai-extensibility.md`, `45-ai-agents-claude.md` · **Effort:** 3 days
...
**Action:** New Haiku agent `AuthenticityRiskScorer` per `45` §17 — scores essays for AI-generated patterns; flag thresholds escalate to integrity signal.

### G-AI5 · DraftSummarizerForReview not implemented
**Severity:** `major` · **Type:** `new` · **Owner:** `32-review-workspace.md`, `45` · **Effort:** 3 days

Per `45` §13 — Opus-tier per-application packet summary for institution reviewers. Currently `StudentDetailPage` has "AI packet summary with regenerate" — confirm it's the same intent and use Opus for the regeneration call.

### G-AI6 · No DiscoveryQueryInterpreter
**Severity:** `major` · **Type:** `new` · **Owner:** `10-discovery.md`, `45` · **Effort:** 2 days
...
**Severity:** `major` · **Type:** `extend` · **Owner:** `42-prompt-library-schema.md` · **Effort:** 10 days
...
3. Wire Adaptive Intake Engine to populate them progressively (per `44`).

### G-D2 · `student_strategies.score_breakdown` and `match_results.match_score` deprecated
**Severity:** `defer` · **Type:** `remove` · **Owner:** `09`, `11` · **Effort:** 1 day
...
**Severity:** `defer` · **Type:** `new` · **Owner:** (future `29`) · **Effort:** 5 days

Connect Peers feature gap (G-S7).

### G-D4 · No `fairness_signals` table
**Severity:** `block` for `46` · **Type:** `new` · **Owner:** `46` · **Effort:** 2 days
...
**Severity:** `block` for institution sales · **Type:** `new` · **Owner:** `46-data-rights-privacy.md` · **Effort:** ongoing
...
**Severity:** `major` (procurement) · **Type:** `new` · **Owner:** `46` · **Effort:** 30+ days (multi-region infra)
...
**Severity:** `minor` · **Type:** `extend` · **Owner:** `36-audit-log.md` · **Effort:** 1 day
...
**Severity:** `block` for `04` rollout · **Type:** `new` · **Owner:** `04` · **Effort:** 2 days
...
**Severity:** `major` · **Type:** `new` · **Owner:** `46` · **Effort:** 0.5 day
...
**Severity:** `major` · **Type:** `new` · **Owner:** `46` · **Effort:** 1 day
