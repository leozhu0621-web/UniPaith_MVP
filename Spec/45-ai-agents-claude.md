# 45 · AI Agents — Per-Agent Prompts & Behavior

> The full agent inventory: existing + planned, with per-agent system prompts, persona blocks, tool schemas, cache strategy, fallback behavior, and output validation. Depends on `04-llm-claude-migration.md` for the provider abstraction and `42-prompt-library-schema.md` for the signal schema each agent reads/writes.
>
...
On failure: `output = agent.fallback(ctx, reason)`. Audit ledger row written either way per `04` §8.
...
**Output:** `{extracted_signals: dict[field_name → {value, confidence}]}` matching subset of `42-prompt-library-schema.md` §3 fields.
...
**THE invariant:** the output schema **mechanically excludes any field that could carry a generated essay or model answer.** Test: `tests/test_workshop_no_generation_contract.py`. See `14-workshops.md`.
...
Env override (per `04` §6): `AI_PROVIDER_PER_AGENT={...}` JSON.
...
Prompt version is written to `ai_artifacts.prompt_version`. Prompt changes trigger cache invalidation (per `04` §12).
