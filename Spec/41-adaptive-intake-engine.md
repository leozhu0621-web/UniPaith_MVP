...
>
> Status: **draft v1.0** · 2026-05-29 · Depends on `40-prompt-library-schema.md` (signal schema) and `42-ai-agents-claude.md` (DiscoveryExtractor, DiscoveryValidator agents).
...
### 3.1 Normalize
- Free text → enum value (e.g., "I want a master's degree" → `target_degree_level=master`). Done by `DiscoveryExtractor` per `42` §3.
...
- Set confidence per the rules in `40-prompt-library-schema.md` §5.
...
Per `40-prompt-library-schema.md` §6 — the engine enforces two gates:

### 4.1 Match-ready (Stage 1 → Stage 2)

Engine computes `match_ready=true` when:
- `core` field coverage ≥ the required subset (per `40` §6.1).
...
Per `40` §3.16 engagement signals.
...
2. The `next_questions_to_ask_user` derived signal (per `40` §4.11) gets a corresponding entry: "Just to confirm — did you mean X?"
...
Privacy: per `43` §2, engagement is gated on `consent.analytics` for cross-cohort aggregates; the student's own dashboard sees their own data unconditionally.
...
5. **Consent respected in flight.** Any agent reading signals checks `consent` per `43`.
...
- **Bulk import for institution-supplied signals.** `22-data-upload.md` covers institution-side bulk import; the engine consumes those rows via the same §3 pipeline (treats them as `institution-supplied` source).
