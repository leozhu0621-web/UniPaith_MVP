# 46 · Data Rights, Privacy, and Fairness Governance
...
2. **Explain everything.** "Every match, every score, every recommendation comes with reasoning." → operationalized in `45-ai-agents-claude.md` rationale agents + `02-design-system.md` §15 (every AI score has a Why affordance).
...
Enforcement points:
- **Inference layer (every Claude call site).** Per `04-llm-claude-migration.md` §11 + `45-ai-agents-claude.md` agent contract.
- **Outbound campaign send.** Per `25-campaigns.md` audience build — `consent.outreach=false` excludes the student.
- **Analytics export pipeline.** Per `28-attribution-funnel-analytics.md` cohort builder — `consent.analytics=false` excludes from cross-cohort aggregates (the student still sees their own data).
- **Training corpus extractor.** Per `44-adaptive-intake-engine.md` provenance — `consent.training=false` excludes from any corpus export.
...
Per `04-llm-claude-migration.md` §8 and `42-prompt-library-schema.md` §4.16 output `audit_ledger_entry_bundle`.
...
- [ ] AI agent path declared in `45-ai-agents-claude.md` (if any).
