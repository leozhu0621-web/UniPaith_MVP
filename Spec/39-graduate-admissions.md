# 39 · Graduate & PhD Admissions

> The graduate-specific admissions layer: faculty-advisor matching, research-interest alignment, funding-package building (TA / RA / fellowship), and department-level review. Graduate admissions is faculty-driven and funding-centric in ways undergraduate isn't — this doc adds that depth on top of the shared pipeline (`30`, `31`, `33`).
>
> Status: **draft v1.0 · Phase-2** · 2026-05-29 · Routes: `/i/admissions?tab=graduate`, department portal `/i/departments/:deptId`, per-applicant `/i/pipeline/:studentId?tab=advisor-match`. Scope: program-type-specific depth, beyond the launch beachhead (`06` §3); sequenced after MVP.
...
- Applicants state research interests + target advisors (`40` §3.12 intent + a grad-specific extension).
- Faculty profiles: research areas, current openings, funding availability, accepting-students flag.
- **Match surface**: for each applicant, ranked advisor-fit (research-interest similarity via embeddings — uses the same vector infra as program matching, `05` §4). Faculty see applicants who fit them; applicants see advisors who fit them.
...
- Structured research-interest taxonomy + free-text statement of purpose parsed (`42` extraction) into interest tags.
- Alignment score per (applicant, advisor) and (applicant, lab/department).
- Feeds the review packet (`31`) as a grad-specific signal — context, not auto-decision (`43`).
...
- Package becomes part of the offer (`33` offer terms extended); student sees it in their offer view (`1A`) + explainer.
- Multi-year package modeling (year-1 fellowship → year-2+ RA, etc.).

### 2.4 Department review portal
- Departments run their own review workspace (a scoped `31`): their applicants, their rubric, their committee.
- Department recommends → central office confirms/releases (`33`) — two-stage, role-gated.
...
| `SoPInterestExtractor` (extends `42`) | Statement of purpose uploaded | Research-interest tags + alignment summary |
...
- Funding package presented to the student as a clear, hopeful package (the one place grad applicants feel the offer's weight — restrained gold permitted on the student's funded-offer moment, `1A`/`02` §15).
...
- Extends `31` (scoped review), `33` (offer terms + funding), `33b` (yield), `40` §3.12 (intent), `05` §4 (vector infra), `42` (extraction).
- Requires program `degree_type` to gate grad-only features.
- Faculty as a new user sub-role (lighter than `institution_admin`) — extends `04` role model; Phase-2 auth work.
...
- Two-stage release: department recommend → central confirm → offer (`33`).
