# 12 · School Detail Page (Student-Facing)
...
### 3.4 Programs
Browse all programs across all schools within this institution. Same chip + filter + sort UX as `10-discovery.md` Discovery. Default scope chip: `Institution · University of Foo`.
...
Both render the same components with an `is_authenticated` prop. Per `47` G-S9 — consolidate to single component.
...
None directly on this page. Insights / similarity surface uses existing program-detail rationale agent (per `11`).
...
- Save / Add to Compare actions consistent with `11`.
