# 11 · Program Detail Page (Student-Facing)
...
>
> Status: **draft v1.0** · 2026-05-29 · Route: `/s/programs/:programId` (also at `/s/schools/:programId` alias — see `47` G-A1 to remove).
...
The current `SchoolDetailPage.tsx` file is mis-named — it's actually the program detail page. Rename per `47` G-A1.
...
#### 3.3a Net Price Estimator (MVP-extend, `49` §2)
A personalized **net price** — not the sticker price — for *this* student at *this* program. Sourced from the OUTPUT schema `42` §4.12 (`net_cost_scenario_range`, `affordability_band`, `aid_scholarship_likelihood_band`).

- Inputs: the program's `cost_data` (COA components) + the student's financial signals (`42` §3.13 budget, `needs_financial_aid_flag`) + scholarship likelihood.
...
- Optional: import a result from a school's official Net Price Calculator (paste/upload) to reconcile against the estimate (`49` Net Price Estimator feature).
...
- Display: in the Costs & Aid tab as a highlighted block; the headline net-price range also appears in the sidebar (§4) and feeds the compare table (`10`/`13`).
