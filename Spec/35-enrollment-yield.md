# 35 · Enrollment Confirmation & Yield

> The tail of the journey (architecture flow Stage 8 — "Enrollment Window / Enrollment Confirmed" student-side; "Enrollment Confirm / Yield Analytics" institution-side). After a student accepts an offer (`18`/`34`), this is where intent is confirmed, deposits are recorded, waitlist movement is managed, and yield is measured. Closes the loop the architecture diagram ends on.
>
> Status: **draft v1.0** · 2026-05-29 · Routes: student `/s/applications/:appId?tab=enrollment`; institution `/i/admissions?tab=yield` + per-applicant `/i/pipeline/:studentId?tab=enrollment`. Splits out of `34` per its §12; closes `49` `35` placeholder.
>
> **Scope boundary:** this doc covers enrollment *confirmation, intent, waitlist, and yield analytics*. The **payment gateway** (real deposit collection, refunds) is deferred to `39-fees-payments.md` (Phase-2). MVP records deposit *status* (paid/waived/pending) set manually or via a future gateway webhook — it does not process money.
...
Appears on the application workspace (`15`) once an offer is `accepted`.

### 2.1 Enrollment checklist
A program-defined post-accept checklist (mirrors the pre-submit checklist pattern in `15`):
- Confirm intent to enroll (single explicit action).
- Pay / mark enrollment deposit (status only in MVP; gateway in `39`).
- Submit final/official transcript.
- Health/immunization forms (if required, `42` §3.2).
- Housing intent form (if offered).
- Visa next steps (if `visa_required_flag`, links to the student's visa readiness from `42` §4.3 — student-facing only).
- Orientation registration.

Each item: status + due date + "what happens if I miss this." Overdue items escalate in Calendar (`16`) + Inbox (`17`).
...
### 2.3 Multiple offers
If the student holds multiple accepted offers (`18` compare), confirming enrollment at one prompts: "You have 2 other active offers. Decline them?" — never auto-declines; the student chooses.
...
### 3.1 Per-applicant
On the review packet (`32`)/pipeline (`31`), an Enrollment tab shows the offer's enrollment state, deposit status, checklist completion, and a timeline. Staff can:
- Record deposit status (paid/waived/pending) — manual in MVP.
- Mark enrollment confirmed.
- Approve a deferral request.
- Send a reason-coded nudge (`29`).

### 3.2 Intent & deposit forms
Institution defines the enrollment-intent form + deposit amount/waiver rules per program/intake (`23`). Student responses land on the per-applicant Enrollment tab.

### 3.3 Waitlist movement
- Waitlist is ranked (`34` §2 `waitlisted` with rank).
- When a seat frees (a decline/withdraw), the dashboard surfaces "N seats open, M on waitlist."
- "Offer to next" → promotes the top-ranked waitlisted applicant → generates an offer (`34` §6, optional human approval) → student notified.
...
A dashboard view (`/i/admissions?tab=yield`) — the analytical end the architecture diagram terminates on. Mirrors/extends `28` patterns.
...
- **Melt** = confirmed-but-not-enrolled (the "summer melt" competitors benchmark; `07` §7).
- **Waitlist conversion** rate.
- **Yield by cohort dimensions** — routed through the fairness lens (`46` §6): yield disparities across protected groups surface here and feed the bias dashboard. (Yield optimization must never become disparate selection.)
- **Predicted final class size** vs target (with `apply_propensity`/`yield_probability` from `42` §4.15 / `4.12`).

Next-best-action: "12 admits haven't confirmed and the deadline is in 5 days → send nudge" (one-click → bulk message `29`).
...
|---|---|---|
| `YieldRiskScorer` (uses `42` §4.15 `apply_propensity`/yield signals) | Dashboard load | Per-admit confirm-probability + at-risk list |
| `NextBestActionForYield` | Dashboard | Ranked actions ("nudge these 12", "release N waitlist") |
| `EnrollmentNudgeDrafter` (reuses `29` `InstitutionReplyDrafter`) | Send nudge | Drafts the confirm-reminder message |

Fairness gate: yield models surface disparities (`46` §6) but never drive selection. Falls back to simple counts on agent failure.
...
- Institution yield dashboard: restrained, data-forward, **no gold** (operational); charts use the `28` palette.
- Fairness/disparity callouts use `--warning`, never alarm-red unless threshold breached (`46`).

---

## 9. Gaps (relative to current code)

- Enrollment state machine + tables NEW (extends the offer model in `34`).
- Yield dashboard NEW (reuses `28` chart infra).
- Waitlist ranking + movement NEW.
- Deposit is **status-only** in MVP — real payment is `39` (Phase-2). Mark clearly in UI ("deposit recorded by institution" vs "pay now").
- `YieldRiskScorer`/`NextBestActionForYield` agents NEW in `45`.
...
- Yield disparity by cohort routes to fairness dashboard (`46`).
...
- **Deposit gateway timing.** MVP records status; when does `39-fees-payments.md` (Stripe/ACH) land? Tie to first institution that requires real collection.
- **Melt intervention.** Automated summer-melt nudges (the Mainstay-style win, `07` §7) — MVP manual nudges; automated cadence Phase-2.
- **SIS handoff.** Enrolled students eventually flow to the institution's SIS (Banner/Workday). Export format + webhook — integration scope, note in `06` §4.
- **Deferral term limits.** Per-institution policy on how far a deferral can push — capture in `23` program config.
