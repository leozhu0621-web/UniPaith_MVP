# 1C · Connect (Student — Stage 3a)

> Stage 3a — Connection & Outreach. The student-facing feed of everything institutions they follow publish: **Updates** (posts), **Events** (RSVP-able), and **Peers** (opt-in connection with other applicants/admits). The consumption mirror of the institution authoring side (`25-posts-updates-events.md`). Lives at `/s/posts`, labeled **Connect** in the top nav.
>
> Status: **draft v1.0** · 2026-05-29 · Route: `/s/posts` (tabs: `?tab=updates|events|peers`). Closes the gap flagged in `04` §4.1 (Connect was mis-pointed at `20`) and `25` §9.
...
Connect is **read/respond**, not authoring. Every item originates from an institution (`25`) or another student (Peers). It is the demand-side endpoint of the institution Outreach module.
...
| Trigger | Effect |
|---|---|
| Save a program (`15`) | Auto-follows that program's institution (toggle in Settings to disable auto-follow). |
| Start an application (`17`) | Auto-follows (cannot disable while application is active). |
| Explicit "Follow" on a program/school detail page (`13`/`14`) | Follows. |
...
- Media (if present).
- CTAs the institution attached (`25` §2.4): View program · RSVP event · Request info · Start application · Add-to-calendar.
- Follow/mute control (top-right).

### 4.2 Ranking
- Default: reverse-chronological.
- Toggle: "Most relevant" → `ConnectFeedRanker` (`42`, see §8) orders by relevance to the student's active applications + saved programs + deadlines approaching.
- Pinned institution posts (`25` §2.2 `pinned`) surface at the top of their institution's items, not the whole feed.
...
- **RSVP** → creates a Calendar item (`18`) + an Inbox confirmation (`19`) + writes `attendees` on the institution event (`25` §3.1). Waitlist if at capacity.
- **Cancel RSVP** → removes calendar item + notifies institution.
- Reminders fire per the institution's follow-up schedule (`25` §3.3) → land in Calendar + Inbox.
...
Connect students with shared application context. **Off by default** — requires explicit opt-in, consent-gated per `43`.
...
- Send connect request → on accept, a peer thread opens in Inbox (`19`) as a `peer` thread type (distinct from `human`/`system`).
- Block/report controls on every peer card. Reports route to a moderation queue.

### 6.4 Safety
- Rate-limit connect requests (anti-spam).
- No minors-to-adults peer matching (gate on `adult_minor_status`, `40` §3.2).
...
|---|---|---|
| `ConnectFeedRanker` (`42` — NEW, see §below) | "Most relevant" toggle / feed load | Ranked feed by relevance to applications + saved + deadlines |
| `EventRecommender` | Events tab | Suggests events on followed programs the student hasn't RSVP'd |

Both fall back to reverse-chronological / unranked if the agent fails or `consent.matching=false`. No AI reads peer data unless `consent.peer_connect` AND `consent.matching` are both true. Add `ConnectFeedRanker` + `EventRecommender` to `42` agent registry (Haiku-tier, cheap ranking).
...
- The route `/s/posts` exists in the IA but no Connect page is implemented (per `04` §4.1; this doc + `90` G-S-Connect).
- `consent.peer_connect` is a NEW consent dimension — add to `43` §2 consent set + `40` §3.2 + the Settings/Data tab.
- `student_follows` table is NEW.
- Peer threads are a NEW `19` thread type (`peer`).
- `ConnectFeedRanker` + `EventRecommender` agents NEW in `42`.
...
- **Deadline items vs Calendar.** Deadline reminders appear in both Connect and Calendar (`18`). Confirm that's desired (redundancy aids recall) vs Calendar-only.
- **Following without saving.** Should a student be able to follow an institution they haven't saved a program from? Yes — explicit follow from the school page (`14`).
