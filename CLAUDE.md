...
- **Detail-page content width is standardized to `max-w-5xl mx-auto`** — the MIT institution page is the "example for all", so institution, college (school-subunit), and BOTH program pages (public + student) use the SAME `max-w-5xl` content container. The width must never change between tabs or across page types. (Set 2026-06-05 — college + student-program were `max-w-6xl`; unified.) **This `max-w-5xl` cap applies ONLY to the editorial Niche-modeled DETAIL pages** — see the app-shell rule below.
