...
export default function NewsGrid({
  ...
  // Pinned posts first (Spec 12 §3.6), then newest-first by date. Events are
  // never pinned, so they sort purely by date among the non-pinned items.
  const isPinned = (it: GridItem) => (it.kind === "post" ? it.pinned : false);
  items.sort((a, b) => Number(isPinned(b)) - Number(isPinned(a)) || b.ts - a.ts);
        ...
        it.kind === "post" ? (
          (() => {
            const inner = (
              <>
                ...
                <div className="p-4">
                  ...
                  {it.dek && (
                    <p className="text-xs text-muted-foreground line-clamp-2 mt-1">{it.dek}</p>
                  )}
            ...
            return it.url ? (
              ...
              <div
                key={it.key}
                className="group block bg-card rounded-xl border border-border overflow-hidden"
              >
                {inner}
              </div>
            );
          ...
          <div key={it.key} className="bg-card rounded-xl border border-border overflow-hidden">
            <div className="aspect-[16/9] bg-gradient-to-br from-secondary/15 to-muted flex flex-col items-center justify-center">
              ...
              {it.date && (
                <span className="mt-1.5 text-xs font-semibold text-foreground">{it.date}</span>
              )}
            </div>
            <div className="p-4">
