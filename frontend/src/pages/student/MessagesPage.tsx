          ...
          <>
              ...
              {msgList.map(msg => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] px-4 py-2 rounded-2xl text-sm ${isOwn ? 'bg-brand-slate-700 text-white rounded-br-md' : 'bg-gray-100 text-brand-slate-700 rounded-bl-md'}`}>
                      {msg.message_body}
                    </div>
                  </div>
                )
            ...
            <div className="px-4 py-3 border-t border-gray-200 bg-white flex gap-2">
              <input
                ...
                className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-slate-700"
              />
              <button onClick={handleSend} disabled={!newMessage.trim()} className="p-2 bg-brand-slate-700 text-white rounded-lg hover:bg-brand-slate-600 disabled:opacity-50">
                <Send size={16} />
              </button>
            </div>
          </>
