  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        {feed.map((item) => {
          if (item.type === 'program') return <ProgramCard key={`p-${item.data.id}`} program={item.data} saved={savedIds.has(item.data.id)} match={matchMap.get(item.data.id)} comparing={compareStore.has(item.data.id)} onSave={() => toggleSave(item.data.id)} onCompare={() => { const p = item.data; if (compareStore.has(p.id)) { compareStore.remove(p.id) } else { compareStore.add({ program_id: p.id, program_name: p.program_name, institution_name: p.institution_name, degree_type: p.degree_type }) } }} onAskCounselor={() => navigate(`/s?prefill=${encodeURIComponent(`Tell me about ${item.data.program_name} at ${item.data.institution_name}. Is it a good fit for me?`)}`)} onView={() => navigate(`/s/programs/${item.data.id}`)} />
