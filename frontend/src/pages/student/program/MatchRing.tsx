...
const TIER_COLORS: Record<number, { ring: string; text: string; label: string; bg: string }> = {
  3: { ring: 'stroke-primary', text: 'text-foreground', label: 'Strong fit', bg: 'bg-primary/10' },
  2: { ring: 'stroke-secondary', text: 'text-secondary', label: 'Good fit', bg: 'bg-secondary/10' },
  1: { ring: 'stroke-secondary', text: 'text-secondary', label: 'Stretch', bg: 'bg-secondary/5' },
...
export default function MatchRing({ score, tier, size = 72 }: Props) {
  ...
  return (
    <div className={`flex items-center gap-3 px-3 py-2 rounded-xl ${color.bg} border border-border`}>
      ...
      <div>
        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Your match</p>
