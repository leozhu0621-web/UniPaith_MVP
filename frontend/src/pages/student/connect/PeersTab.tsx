  ...
  if (!status.opted_in) {
    return (
      <div className="bg-card rounded-xl border border-border p-6 text-center max-w-md mx-auto">
        ...
        <ul className="text-xs text-foreground text-left max-w-xs mx-auto mb-4 space-y-1">
...
function EmptyExplainer({ title, body }: { title: string; body: string }) {
  return (
    <div className="text-center py-12">
      ...
      <p className="text-xs text-foreground max-w-sm mx-auto">{body}</p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-foreground">Applicants to programs you're considering.</p>
        <button
    ...
    <div className="bg-card rounded-xl border border-border p-4 flex flex-col">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {peer.intended_major && <p className="text-xs text-foreground truncate">{peer.intended_major}</p>}
          {peer.region && (
            <p className="text-[10px] text-foreground flex items-center gap-0.5 mt-0.5">
              <MapPin size={9} /> {peer.region}
            </p>
          )}
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          <button onClick={onReport} title="Report" className="text-foreground hover:text-error p-1"><Flag size={13} /></button>
          <button onClick={onBlock} title="Block" className="text-foreground hover:text-error p-1"><Ban size={13} /></button>
        </div>
      </div>

      {peer.bio && <p className="text-xs text-foreground mt-2 line-clamp-2">{peer.bio}</p>}

      {peer.shared_programs.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          {peer.shared_programs.slice(0, 3).map(sp => (
            <span key={sp.id} className="px-2 py-0.5 text-[10px] rounded-full bg-muted text-foreground">{sp.name}</span>
          ))}
        </div>
      )}
      ...
      <div className="mt-3 pt-3 border-t border-border">
        {peer.connection_state === 'connected' ? (
          ...
          <span className="text-xs text-foreground">Request sent</span>
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-4 space-y-3">
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button onClick={onClose} className="px-4 py-1.5 text-xs font-medium rounded-lg text-foreground hover:text-foreground">Cancel</button>
      </div>
    </div>
  )
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="text-[10px] font-medium text-foreground uppercase tracking-wider">{label}</span>
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)} className="flex items-center gap-2 text-xs text-foreground">
