  ...
  return (
    <Card variant="card-raised" className="flex flex-col gap-2">
      <div className="flex items-start justify-between gap-2">
        <h4 className="text-sm font-semibold leading-snug text-student-ink">{story.title}</h4>
        <div className="flex shrink-0 gap-1">
          <button
            type="button"
            onClick={() => onEdit(story)}
            className="rounded p-1 text-student-text transition-colors hover:bg-student-mist hover:text-cobalt"
          ...
          <button
            ...
            className="rounded p-1 text-student-text transition-colors hover:bg-error-soft hover:text-error"
      ...
      {story.summary && (
        <p className="line-clamp-2 text-xs text-student-text">{story.summary}</p>
      )}
      ...
      <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-1">
        ...
        {(story.context_tags ?? []).slice(0, 3).map(t => (
          <span
            key={t}
            className="rounded-full bg-student-mist px-2 py-0.5 text-xs capitalize text-student-text"
          >
            {t}
          </span>
        ))}
        {story.difficulty_tier != null && (
          <span className="inline-flex items-center gap-0.5 text-xs text-student-text">
