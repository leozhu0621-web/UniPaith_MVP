...
export function useJourneyState(enabled: boolean) {
  ...
  // Both queries must have resolved before milestone detection baselines, or the
  // empty loading state is mistaken for "nothing done yet" on a returning student.
  const loaded = completion !== undefined && handoff !== undefined;
  return { stages, currentStage, matchesUnlocked, completion, loaded };
}
