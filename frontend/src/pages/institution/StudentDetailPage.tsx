...
import { makeDecision, createOffer } from '../../api/applications-admin'
...
import type {
  Rubric, ReviewPacket, ReviewSynthesis, InstitutionMatchRationale, IntegrityAction,
} from '../../types'
...
export default function StudentDetailPage() {
  ...
  const decisionMut = useMutation({
    mutationFn: () => makeDecision(applicationId!, { decision: decision as 'admitted' | 'rejected' | 'waitlisted' | 'deferred', decision_notes: decisionNotes || null }),
    onSuccess: () => { showToast('Decision recorded', 'success'); setShowDecisionModal(false); invalidatePacket() },
  ...
  const offerMut = useMutation({
    ...
    onSuccess: () => { showToast('Offer created', 'success'); setShowOfferModal(false); invalidatePacket() },
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <Card className="p-4 space-y-2">
            ...
            <Button variant="tertiary" className="w-full flex items-center justify-center gap-2" onClick={() => { setDecision('deferred'); setShowDecisionModal(true) }} disabled={locked}><Clock size={16} /> Mark for committee</Button>
            <Button className="w-full flex items-center justify-center gap-2" onClick={() => setShowDecisionModal(true)} disabled={locked}><Award size={16} /> Make decision</Button>
            {packet.decision?.decision === 'admitted' && (
              <Button variant="secondary" className="w-full flex items-center justify-center gap-2" onClick={() => setShowOfferModal(true)}><FileText size={16} /> Create offer</Button>
            )}
          </Card>
        </div>
      ...
      <Modal isOpen={showDecisionModal} onClose={() => setShowDecisionModal(false)} title="Make decision">
        <div className="space-y-4">
          <div className="space-y-2">
            {DECISION_OPTIONS.map(opt => (
              <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="decision" value={opt.value} checked={decision === opt.value} onChange={e => setDecision(e.target.value)} className="accent-cobalt" />
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant={decision === 'rejected' ? 'destructive' : 'primary'} onClick={() => decisionMut.mutate()} disabled={!decision || decisionMut.isPending}>{decisionMut.isPending ? 'Saving…' : 'Confirm decision'}</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={showOfferModal} onClose={() => setShowOfferModal(false)} title="Create offer">
        <div className="space-y-4">
          <Select label="Offer type" options={[
          ...
          <div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setShowOfferModal(false)}>Cancel</Button><Button onClick={() => offerMut.mutate()} disabled={offerMut.isPending}>{offerMut.isPending ? 'Creating…' : 'Create offer'}</Button></div>
        </div>
      </Modal>
