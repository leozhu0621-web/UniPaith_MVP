...
const journeyStages = [
  { num: '01', title: 'Tell Us About You', desc: 'Goals, budget, location, timeline — a quick conversation, not a 20-page form.', color: 'bg-student' },
  { num: '02', title: 'Spot the Blind Spots', desc: 'We flag things you might miss: visa requirements, hidden costs, eligibility gaps.', color: 'bg-charcoal' },
  { num: '03', title: 'Lock In Your Priorities', desc: 'Cost vs ranking, location vs timeline — you decide the tradeoffs, we respect them.', color: 'bg-student' },
  { num: '04', title: 'Build Your Shortlist', desc: 'Programs matched to your criteria. You\'ll see why each one made the cut.', color: 'bg-charcoal' },
  { num: '05', title: 'Get Application-Ready', desc: 'Checklists, documents, essays — everything organized with clear next steps.', color: 'bg-student' },
  { num: '06', title: 'Apply From One Dashboard', desc: 'Submit to multiple programs without re-entering the same info on 12 portals.', color: 'bg-charcoal' },
  { num: '07', title: 'Track Every Response', desc: 'Status updates, interview invites, and decisions — all in one feed.', color: 'bg-student' },
  { num: '08', title: 'Compare Offers & Decide', desc: 'Side-by-side comparison with real costs, conditions, and deadlines.', color: 'bg-charcoal' },
]

const features = [
  { icon: UserCircle, title: 'One Profile, Every School', desc: 'Fill it out once. Academics, activities, essays, documents — reuse across every application.', color: 'text-student', bg: 'bg-student-mist' },
  { icon: Search, title: 'Smart Matching', desc: 'Programs ranked by how well they fit your goals, budget, and profile — with clear reasoning.', color: 'text-student', bg: 'bg-student-mist' },
  { icon: Send, title: 'Application Tracker', desc: 'Deadlines, checklists, status — stop juggling spreadsheets and email tabs.', color: 'text-charcoal', bg: 'bg-charcoal/10' },
  { icon: FileText, title: 'Essay Workshop', desc: 'Get feedback on your essays, track versions, and tailor drafts per program.', color: 'text-charcoal', bg: 'bg-charcoal/10' },
  { icon: DollarSign, title: 'Cost & Aid Breakdown', desc: 'Compare real costs across programs. Find scholarships you qualify for.', color: 'text-student', bg: 'bg-student-mist' },
  { icon: Target, title: 'Readiness Check', desc: 'Know where you stand before you hit submit. Strengths, gaps, and what to fix.', color: 'text-student', bg: 'bg-student-mist' },
]
...
export default function ForStudentsPage() {
  return (
    <div className="pt-16">
      {/* Hero */}
      <section className="relative py-24 sm:py-32 px-4 sm:px-6 lg:px-8 overflow-hidden bg-offwhite">
        <div className="absolute inset-0 -z-10">
          <div className="absolute top-20 left-[10%] w-72 h-72 bg-student-mist rounded-full blur-3xl animate-float" />
          <div className="absolute bottom-20 right-[10%] w-96 h-96 bg-gold-pale/40 rounded-full blur-3xl animate-float-slow" />
        </div>
        <div className="max-w-4xl mx-auto text-center">
          <ScrollReveal variant="blur-in">
            <span className="inline-flex items-center gap-2 bg-student-mist text-student rounded-full px-5 py-2 text-sm font-medium mb-8">
              Everything a $6K agent does &mdash; for free
            </span>
          </ScrollReveal>
          <ScrollReveal delay={200} variant="scale-up">
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-charcoal leading-[1.1] mb-6 tracking-tight font-heading">
              Find the right school.{' '}
              <br className="hidden sm:block" />
              <span className="text-student">Get in.</span>
            </h1>
          </ScrollReveal>
        ...
        <div className="max-w-5xl mx-auto">
          <ScrollReveal variant="blur-in">
            <div className="text-center mb-14">
              <h2 className="text-3xl sm:text-4xl font-bold text-charcoal mb-3 font-heading">Here&rsquo;s what happens when you sign up</h2>
              ...
              {journeyStages.map((stage, i) => (
                <ScrollReveal key={i} delay={i * 80} variant="fade-left">
                  <div className="flex gap-6 items-start">
                    ...
                    <div className="flex-1 bg-offwhite rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
                      <h3 className="text-base font-bold text-charcoal mb-0.5 font-heading">{stage.title}</h3>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-offwhite">
        <div className="max-w-6xl mx-auto">
          <ScrollReveal variant="blur-in">
            <div className="text-center mb-14">
              <h2 className="text-3xl sm:text-4xl font-bold text-charcoal mb-3 font-heading">What you get</h2>
            ...
            {features.map((f, i) => (
              <ScrollReveal key={i} delay={i * 80} variant="fade-up">
                <div className="bg-white rounded-2xl border border-gray-200 p-7 hover:shadow-md transition-all h-full group">
                  ...
                  <h3 className="text-lg font-bold text-charcoal mb-2 font-heading">{f.title}</h3>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-charcoal">
        <div className="max-w-5xl mx-auto">
          <ScrollReveal variant="blur-in">
            <div className="text-center mb-14">
              <span className="inline-flex items-center gap-1.5 bg-student/20 text-student text-sm font-medium rounded-full px-5 py-1.5 mb-4">
          ...
          <ScrollReveal delay={200}>
            <div className="max-w-2xl mx-auto bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
              <div className="px-6 py-3 border-b border-white/10 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-student flex items-center justify-center"><Sparkles size={16} className="text-white" /></div>
                ...
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.from === 'student' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                      msg.from === 'student' ? 'bg-student text-white rounded-br-sm' : 'bg-white/10 text-gray-200 rounded-bl-sm'
                    }`}>{msg.text}</div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-offwhite">
        <div className="max-w-4xl mx-auto text-center">
          <ScrollReveal variant="blur-in">
            <h2 className="text-3xl sm:text-4xl font-bold text-charcoal mb-3 font-heading">You stay in control</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={i} className="flex flex-col items-center gap-2 p-4">
                  <badge.icon size={24} className="text-student" />
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-3xl mx-auto">
          <ScrollReveal variant="blur-in">
            <h2 className="text-3xl font-bold text-center text-charcoal mb-10 font-heading">Common questions</h2>
          </ScrollReveal>
          <ScrollReveal delay={200}>
            <Accordion type="single" collapsible className="space-y-3">
              {faqs.map((faq, i) => (
                <AccordionItem key={i} value={`faq-${i}`} className="bg-offwhite rounded-xl border border-gray-200 px-6">
                  <AccordionTrigger className="text-left font-semibold text-charcoal hover:no-underline py-4 text-sm">{faq.q}</AccordionTrigger>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-charcoal">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <ScrollReveal delay={200}>
            <Button size="lg" className="text-base px-12 py-7 rounded-xl shadow-lg text-lg bg-student hover:bg-student-hover text-white" asChild>
              <Link to="/signup?role=student">Create Your Profile <ArrowRight size={20} className="ml-2" /></Link>
            </Button>
          </ScrollReveal>
        </div>
